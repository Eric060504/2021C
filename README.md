...
| 问题 3 | `q3_cost_compression_plan.py` | A/C 材料偏好、采购奖惩、低损耗率硬约束、敏感性分析 | 最终附件 A/B 的“问题 3”工作表、`outputs/q3/sensitivity_analysis.csv` |
...
4. 敏感性分析会同时检查材料比例、库存可行性和低损耗转运可行性。

当前结果中，问题 3 使用 T3、T4、T6、T8 四家转运商，已使用转运商的最大预测损耗率为 0.678%，低于 2% 阈值。
